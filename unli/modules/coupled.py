...
import torch
from unli.modules import Joiner, BERTConcatenator, BertSeq2VecEncoderForPairs , BartSeq2VecEncoderForPairs
...
class CoupledSentencePairFeatureExtractorBart(SentencePairFeatureExtractor):
    def __init__(self,
                 joiner: Joiner,
                 encoder: Seq2VecEncoder,
                 l_token_index_field: str = "wordpiece",
                 r_token_index_field: str = "wordpiece"):
        super().__init__()
        print(f"CoupledSentencePairFeatureExtractor constructor")
    ...
    @classmethod
    def from_params(cls, vocab, params):
        print(f"CoupledSentencePairFeatureExtractor params {params}")

        return cls(
            joiner={
                "bart_concat": lambda: BERTConcatenator()
            }[params["joiner"]](),
            encoder=BartSeq2VecEncoderForPairs.from_params(params)
        )
